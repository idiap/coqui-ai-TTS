...
class HGST(nn.Module):
    ...
    def __init__(self, num_mel, num_heads, num_style_tokens, gst_embedding_dim, embedded_speaker_dim=None, num_hierarchy_layers=3):
        super().__init__()
        self.encoder = ReferenceEncoder(num_mel, gst_embedding_dim)
        
        self.style_token_layers = nn.ModuleList([StyleTokenLayer(num_heads, num_style_tokens, gst_embedding_dim, embedded_speaker_dim) for i in range(num_hierarchy_layers)])
    def forward(self, inputs, speaker_embedding=None):
        ...
        style_embed = None
        for i, layer in enumerate(self.style_token_layers):
            # first layer
            if i == 0:
                style_embed = layer(enc_out, r=None)
            else:
                style_embed = layer(style_embed, r=enc_out)
...
class StyleTokenLayer(nn.Module):
    ...
    def forward(self, inputs, r=None):
        ...
        if r:
            prosody_encoding = (r - inputs).unsqueeze(1)
        else:
