...
class AudioProcessor:
    ...
    Args:
        config: BaseAudioConfig instance
    """

    def __init__(self, config: BaseAudioConfig) -> None:
        ...
        self.griffin_lim_iters = config.griffin_lim_iters
        self.signal_norm = config.signal_norm
        self.symmetric_norm = config.symmetric_norm
        self.mel_fmin = config.mel_fmin
        self.mel_fmax = config.mel_fmax
        self.pitch_fmin = config.pitch_fmin
        self.pitch_fmax = config.pitch_fmax
        self.spec_gain = float(config.spec_gain)
        self.stft_pad_mode = config.stft_pad_mode
        self.max_norm = float(config.max_norm)
        self.clip_norm = config.clip_norm
        self.do_trim_silence = config.do_trim_silence
        self.trim_db = config.trim_db
        self.do_sound_norm = config.do_sound_norm
        self.do_amp_to_db_linear = config.do_amp_to_db_linear
        self.do_amp_to_db_mel = config.do_amp_to_db_mel
        self.do_rms_norm = config.do_rms_norm
        self.db_level = config.db_level
        self.stats_path = config.stats_path
        # setup exp_func for db to amp conversion
        if config.log_func == "np.log":
            self.base = np.e
        elif config.log_func == "np.log10":
            self.base = 10
        ...
        assert self.min_level_db != 0.0, " [!] min_level_db is 0"
        ...
        # setup scaler
        if self.stats_path and self.signal_norm:
            mel_mean, mel_std, linear_mean, linear_std, _ = self.load_stats(self.stats_path)
